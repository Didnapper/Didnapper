...
class Window_Message < Window_Selectable
  ...
  def reset_window (change_opacity = true)
    ...
    if $game_temp.in_battle
      if $game_switches[319] == true
        self.y = 200
      else 
        self.y = 16 #Kenrian - original is 16
      end
    else
      if $game_system.message_event == -1 and $game_system.train_actor == -1
    ...
    if $game_switches[561] == true #Martin/Kendrian If private cell is open
      self.x =  $game_variables[407] # 0
      self.y =  $game_variables[408] # 370
    elsif self.x < 0 #Original: if
      self.x = 0
